<template>
  <div class="dr-contentTagForm">
    <a-modal :xs="20" :sm="20" :md="6" :lg="6" :xl="6" :title="$t('contentTag.form_title')" @Ok="submitForm"
      v-model:open="props.dialogState.show"
      :okText="dialogState.edit ? $t('main.form_btnText_update') : $t('main.form_btnText_save')">
      <a-form :model="props.dialogState.formData" :rules="rules" ref="modalFormRef" label-width="120px"
        class="demo-ruleForm" :label-position="device == 'mobile' ? 'top' : 'right'">
        ...
        <a-form-item :label="$t('contentTag.comments')" prop="comments">
          <a-input size="small" type="textarea" v-model:value="props.dialogState.formData.comments"></a-input>
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
