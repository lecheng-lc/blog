<template>
  <div>
    <a-modal width="30%" :title="$t('adminUser.lb_form_title')" v-model:open="dialogState.show"
      ...
      <a-form ref="modalFormRef" :model="dialogState.formData" :rules="rules" label-width="120px" class="demo-ruleForm"
        ...
        <a-form-item label="头像" prop="logo">
          <a-upload class="avatar-uploader" :action="uploadFile" :show-file-list="false" list-type="picture-card"
            @change="handleAvatarSuccess" :before-upload="beforeAvatarUpload" :data="{ action: 'uploadimage' }">
            {{ dialogState.formData.logo }}
            <img v-if="props.dialogState.formData.logo" :src="props.dialogState.formData.logo" class="avatar" />
            <div v-else>
        ...
        <a-form-item :label="$t('adminUser.lb_password')" name="password">
          <a-input type="password" :placeholder="$t('main.noModifyPasswordTips')"
            v-model:value="props.dialogState.formData.password"></a-input>
        </a-form-item>
        <a-form-item :label="$t('adminUser.lb_userGroup')" name="group">
          {{ props.dialogState.formData.group }}
          <a-select v-model:value="props.dialogState.formData.group" :placeholder="$t('main.ask_select_label')">
            <a-select-option :key="index" v-for="(group, index) in props.groups" :label="group.name" :value="group._id">{{
              group.name }}</a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item :label="$t('adminUser.lb_countryCode')" prop="countryCode">
          <a-select size="small" v-model:value="dialogState.formData.countryCode" placeholder="国家代码">
            <a-option v-for="item in countryCode" :key="'kw_' + item.value" :label="item.label"
        ...
        <a-form-item :label="$t('adminUser.lb_comments')" prop="comments">
          <a-input size="small" type="textarea" v-model:value="dialogState.formData.comments"></a-input>
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
...
const props = defineProps<{
  dialogState: typeof userStoreIns.formState
  device: string
  groups: typeof groupStoreIns.roleList.docs
}>()
...
const handleAvatarSuccess = (info: UploadChangeParam) => {
  if (info.file.status === 'done') {
    let imageUrl = info.file.thumbUrl
...
const resetForm = () => {
  props.dialogState.formData = {} as any
}
  ...
  modalFormRef.value
    ...
    .then(async () => {
      ...
      if (props.dialogState.edit) {
        const [err, res] = await updateAdminUser(params as unknown as Interface.UserUpdateOneReq)
        if (res) {
        ...
        const [err, res] = await addAdminUser(params as unknown as Interface.UserAddOneReq)
        if (res) {
          userStoreIns.hideAdminUserForm()
          userStoreIns.getAdminUserList()
